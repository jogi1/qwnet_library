...
static unsigned int NET_StringToNetAddrNoPort(struct qw_connection *qwc, struct SysNetData *sysnetdata, const char *s, struct netaddr *a)
{
	unsigned int len;

	if (sysnetdata == 0)
		sysnetdata = qwc->netdata->sysnetdata;
...
qboolean NET_StringToAdr(struct qw_connection *qwc, struct SysNetData *sysnetdata, const char *s, struct netaddr *a)
{
	...
	len = NET_StringToNetAddrNoPort(qwc, sysnetdata, s, a);
	if (len == 0)
		return false;
...
char *NET_GetHostnameForAddress(struct qw_connection *qwc, const struct netaddr *addr)
{
	...
	if (Sys_Net_ResolveAddress(qwc->netdata->sysnetdata, addr, buf, sizeof(buf)))
		return buf;
...
static qboolean NET_GetLoopPacket(struct qw_connection *qwc, enum netsrc sock, struct sizebuf_s *message, struct netaddr *from)
{
	int i;
	struct loopback *loop;

	loop = &qwc->netdata->loopbacks[sock];
...
static void NET_SendLoopPacket(struct qw_connection *qwc, enum netsrc sock, int length, void *data, const struct netaddr *to)
{
	int i;
	struct loopback *loop;

	loop = &qwc->netdata->loopbacks[sock ^ 1];

	i = loop->send & (MAX_LOOPBACK - 1);
	loop->send++;

	if (length > sizeof(loop->msgs[i].data))
		Sys_Error (qwc, "NET_SendLoopPacket: length > MAX_UDP_PACKET");
...
{
	if (qwc->netdata == 0)
		Sys_Error(qwc, "NET_ClearLoopback() called before net init\n");

	qwc->netdata->loopbacks[0].send = qwc->netdata->loopbacks[0].get = 0;
	qwc->netdata->loopbacks[1].send = qwc->netdata->loopbacks[1].get = 0;
}
...
{
	qwc->netdata = malloc(sizeof(struct NetData));
	if (qwc->netdata)
	{
		memset(qwc->netdata, 0, sizeof(struct NetData));

		qwc->netdata->sysnetdata = Sys_Net_Init();
		if (qwc->netdata->sysnetdata == 0)
...
{
	if (qwc->netdata == 0)
		Sys_Error(qwc, "NET_Shutdown() called twice\n");

	if (qwc->netdata->sockets[NS_CLIENT])
		Sys_Net_DeleteSocket(qwc->netdata->sysnetdata, netdata->sockets[NS_CLIENT]);

	NET_ServerConfig(qwc, false);

	Sys_Net_Shutdown(qwc->netdata->sysnetdata);

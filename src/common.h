...
void *SZ_GetSpace (struct sizebuf_s *buf, int length);
void SZ_Write (struct sizebuf_s *buf, void *data, int length);
void SZ_Print (struct sizebuf_s *buf, char *data);	// strcats onto the sizebuf
...
void MSG_WriteString (struct sizebuf_s *sb, char *s);
...
void MSG_WriteDeltaEntity  (entity_state_t *from, entity_state_t *to, struct sizebuf_s *msg, qboolean force);
void MSG_EmitPacketEntities (packet_entities_t *from, int delta_sequence, packet_entities_t *to, 
	struct sizebuf_s *msg, entity_state_t *(*GetBaseline)(int number)); 
...
float MSG_ReadFloat (struct qw_connection *qwcl);

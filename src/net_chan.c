...
static void Huff_CompressPacketSizebuf(struct HuffContext *huffcontext, struct sizebuf_s *msg, int offset)
{
...
static void Huff_DecompressPacketSizebuf(struct HuffContext *huffcontext, struct sizebuf_s *msg, int offset)
{
	char buf[MAX_MSGLEN+1];
...
void Netchan_Setup (enum netsrc sock, struct netchan_s *chan, struct netaddr adr, int qport)
{
...
qboolean Netchan_CanPacket (struct netchan_s *chan)
{
...
qboolean Netchan_CanReliable (struct netchan_s *chan)
{
...
void Netchan_Transmit (struct netchan_s *chan, int length, byte *data)
{
	struct sizebuf_s send;
	byte send_buf[MAX_MSGLEN + PACKET_HEADER];
...
qboolean Netchan_Process(struct qw_connection *qwc)
{
	unsigned sequence, sequence_ack, reliable_ack, reliable_message;
    struct netchan_s *chan;
    struct sizebuf_s *message;

    chan = &qwc->netchan;
    message = &qwc->net_message;

	// get sequence numbers		
	MSG_BeginReading(qwc);
	sequence = MSG_ReadLong (qwc);
	sequence_ack = MSG_ReadLong (qwc);

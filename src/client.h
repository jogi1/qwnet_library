...
extern struct sizebuf_s cl_net_message;
...
ca_connected,		// struct netchan_s established, waiting for svc_serverdata
...
void CL_WriteDemoMessage(const struct sizebuf_s *msg);
...
void CL_RequestNextFTEDownloadChunk(struct sizebuf_s *buf);
...
void MT_Update_Hud_Info(void);
int MT_NumTeams(void);
int MT_Value(void);
char *MT_Name(void);

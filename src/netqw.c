...
static int NetQW_Thread_Init(struct qw_connection *qwc, struct NetQW *netqw)
{
	qboolean b;
	...
	if (netqw->netdata)
	{
		b = NET_StringToAdr(qwc, netqw->netdata, netqw->hoststring, &netqw->addr);
		if (!b)
		{
...
struct NetQW *NetQW_Create(struct qw_connection *qwc, const char *hoststring, const char *userinfo, unsigned short qport)
{
	struct NetQW *netqw;
	int r;
	...
	if (netqw)
	{
		...
		if (netqw->hoststring)
		{
			...
			if (netqw->userinfo)
			{
				...
				if (netqw->mutex)
				{
					netqw->thread = Sys_Thread_CreateThread(NetQW_Thread, qwc);
					if (netqw->thread)
					{
...
void NetQW_CopyFrames(struct NetQW *netqw, struct frame_s *frames, unsigned int *newseqnr, unsigned int *startframe, unsigned int *numframes)
{
...
enum state NetQW_GetState(struct NetQW *netqw)
{
    if (netqw == NULL)
